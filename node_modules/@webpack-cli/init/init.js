 ...
 */
const mapOptionsToTransform = (config) => Object.keys(config.webpackOptions)
    .filter((key) => prop_types_1.default.has(key));
/**
 *
...
function runTransform(webpackProperties, action) {
    ...
    const webpackConfig = Object
        ...
        .filter((p) => p !== "configFile" && p !== "configPath");
    ...
    webpackConfig.forEach((scaffoldPiece) => {
        ...
        const ast = j(initActionNotDefined
    ...
    if (initActionNotDefined && webpackProperties.config.item) {
        process.stdout.write("\n" +
            chalk_1.default.green(`Congratulations! ${webpackProperties.config.item} has been ${action}ed!\n`));
    }
    else {
        process.stdout.write("\n" +
            chalk_1.default.green("Congratulations! Your new webpack configuration file has been created!\n"));
    }
}
